...
void main(void)
{
	...
	vec3 reflectedLight = vec3(0.0);
		...
		vec4 applied_Texture_Col = texture( Texture_Sample_1, ex_UVCoords.xy );
	
		vec3 rgb = min(texture2D(Texture_Sample_1 , ex_UVCoords.xy ).rgb * scatteredLight + reflectedLight, vec3(1.0) );
	
		out_Color = vec4(rgb, applied_Texture_Col);
